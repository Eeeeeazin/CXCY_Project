...
def genMsgLengthDescriptor(msg_bitsLenth):
    ...
    '''
    return __import__("struct").pack(">Q",msg_bitsLenth)
...
def reverse_hex_8bytes(hex_str):
    ...
    '''
    hex_str_ = hex_str.decode().strip('\x00').strip('`').strip('').encode()
    hex_str = "%016x"%int(hex_str_,16)
    assert len(hex_str)==16    
    return __import__("struct").pack("<Q",int(hex_str,16)).encode("utf-8")
...
def reverse_hex_4bytes(hex_str):
    ...
    return __import__("struct").pack("<L",int(hex_str,16)).encode("utf-8")
...
def deal_rawInputMsg(input_msg):
    ...
    '''
    ascii_list = [x.encode("utf-8") for x in input_msg]
