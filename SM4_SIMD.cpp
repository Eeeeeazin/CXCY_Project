...
#define MM256_EPI32_0(a, b, c, d) _mm256_unpacklo_epi64(_mm256_unpacklo_epi32(a, b),_mm256_unpacklo_epi32(c, d))
#define MM256_EPI32_1(a, b, c, d) _mm256_unpackhi_epi64(_mm256_unpacklo_epi32(a, b),_mm256_unpacklo_epi32(c, d))
#define MM256_EPI32_2(a, b, c, d) _mm256_unpacklo_epi64(_mm256_unpackhi_epi32(a, b),_mm256_unpackhi_epi32(c, d))
#define MM256_EPI32_3(a, b, c, d) _mm256_unpackhi_epi64(_mm256_unpackhi_epi32(a, b),_mm256_unpackhi_epi32(c, d))
...
void SM4_Encrypt(uint8_t* cin, uint8_t* out, uint32_t* sm4_key)
{
    __m256i x[4];
    __m256i temp[4];
    __m256i ff;
    __m256i* cin_ = (__m256i*)cin;
    ff = _mm256_set1_epi32(0xFF);
    temp[0] = _mm256_loadu_si256(cin_ + 0);
    temp[1] = _mm256_loadu_si256(cin_ + 1);
    temp[2] = _mm256_loadu_si256(cin_ + 2);
    temp[3] = _mm256_loadu_si256(cin_ + 3);
    x[0] = MM256_EPI32_0(temp[0], temp[1], temp[2], temp[3]);
    x[1] = MM256_EPI32_1(temp[0], temp[1], temp[2], temp[3]);
    x[2] = MM256_EPI32_2(temp[0], temp[1], temp[2], temp[3]);
    x[3] = MM256_EPI32_3(temp[0], temp[1], temp[2], temp[3]);
    __m256i vindex = _mm256_setr_epi8(2, 3, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12, 2, 3, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12);
    ...
    for (int i = 0; i < 32; i++)
    {
        ...
        temp[1] = _mm256_xor_si256(x[0], _mm256_i32gather_epi32((const int*)Table0, _mm256_and_si256(temp[0], ff), 4));
        temp[0] = _mm256_srli_epi32(temp[0], 8);
        temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi32((const int*)Table1, _mm256_and_si256(temp[0], ff), 4));
        temp[0] = _mm256_srli_epi32(temp[0], 8);
        temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi32((const int*)Table2, _mm256_and_si256(temp[0], ff), 4));
        temp[0] = _mm256_srli_epi32(temp[0], 8);
        temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi32((const int*)Table3, _mm256_and_si256(temp[0], ff), 4));
        x[0] = x[1];
        x[1] = x[2];
        x[2] = x[3];
        x[3] = temp[1];
    ...
    x[2] = _mm256_shuffle_epi8(x[2], vindex);
    x[3] = _mm256_shuffle_epi8(x[3], vindex);
    _mm256_storeu_si256((__m256i*)out + 0, MM256_EPI32_0(x[3], x[2], x[1], x[0]));
    _mm256_storeu_si256((__m256i*)out + 1, MM256_EPI32_1(x[3], x[2], x[1], x[0]));
    _mm256_storeu_si256((__m256i*)out + 2, MM256_EPI32_2(x[3], x[2], x[1], x[0]));
    _mm256_storeu_si256((__m256i*)out + 3, MM256_EPI32_3(x[3], x[2], x[1], x[0]));
}

void SM4_Decrypt(uint8_t* out, uint8_t* cin, uint32_t* sm4_key)
{
    __m256i x[4];
    __m256i temp[4];
    __m256i ff;
    __m256i* out_ = (__m256i*)out;
    ff = _mm256_set1_epi32(0xFF);
    temp[0] = _mm256_loadu_si256(out_ + 0);
    temp[1] = _mm256_loadu_si256(out_ + 1);
    temp[2] = _mm256_loadu_si256(out_ + 2);
    temp[3] = _mm256_loadu_si256(out_ + 3);
    x[0] = MM256_EPI32_0(temp[0], temp[1], temp[2], temp[3]);
    x[1] = MM256_EPI32_1(temp[0], temp[1], temp[2], temp[3]);
    x[2] = MM256_EPI32_2(temp[0], temp[1], temp[2], temp[3]);
    x[3] = MM256_EPI32_3(temp[0], temp[1], temp[2], temp[3]);
    ...
    for (int i = 0; i < 32; i++)
    {
        __m256i k = _mm256_set1_epi32(sm4_key[31 - i]);
        temp[0] = _mm256_xor_si256(_mm256_xor_si256(x[1], x[2]), _mm256_xor_si256(x[3], k));
        temp[1] = _mm256_xor_si256(x[0], _mm256_i32gather_epi32((const int*)Table0, _mm256_and_si256(temp[0], ff), 4));
        temp[0] = _mm256_srli_epi32(temp[0], 8);
        temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi32((const int*)Table1, _mm256_and_si256(temp[0], ff), 4));
    ...
    x[2] = _mm256_shuffle_epi8(x[2], vindex);
    x[3] = _mm256_shuffle_epi8(x[3], vindex);
    _mm256_storeu_si256((__m256i*)cin + 0, MM256_EPI32_0(x[3], x[2], x[1], x[0]));
    _mm256_storeu_si256((__m256i*)cin + 1, MM256_EPI32_1(x[3], x[2], x[1], x[0]));
    _mm256_storeu_si256((__m256i*)cin + 2, MM256_EPI32_2(x[3], x[2], x[1], x[0]));
    _mm256_storeu_si256((__m256i*)cin + 3, MM256_EPI32_3(x[3], x[2], x[1], x[0]));
}

int main()
{
    unsigned char key[128] = { 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb };
    unsigned char cin[128] = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10 };
    uint32_t* sm4_key = (uint32_t*)malloc(32 * sizeof(uint32_t));
    if (sm4_key)
    {
        uint32_t k[4];
        uint32_t tmp;
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                tmp_8[j] = S_box[tmp_8[j]];
            }
            sm4_key[i] = k[0] ^ tmp ^ (tmp << 13) ^ (tmp >> 23);
            k[0] = k[1];
            k[1] = k[2];
            k[2] = k[3];
            k[3] = sm4_key[i];
        ...
        {
            SM4_Encrypt(cin, cin, sm4_key);
        }
        double end = clock();
        cout << "加密时间:" << end - start << "ms" << endl;
        start = clock(); 
        for (int i = 0; i < 1000; i++)
        {
            SM4_Decrypt(cin, cin, sm4_key);
        }
        end = clock();
        cout << "解密时间:" << end - start << "ms" << endl;
        free(sm4_key);
    }
    return 0;
}